...
# Polymarket CLOB uses NATIVE USDC (0x3c49...), not USDC.e (0x2791...)
USDC_NATIVE = Web3.to_checksum_address("0x3c499c512c044ca2335a4aabd539265f21ce3341")
...
def init_clob():
    try:
        # SIGNATURE_TYPE: 1 for Email/Magic (Proxy), 0 for Direct EOA
        sig_type = int(os.getenv("SIGNATURE_TYPE", 1))
        # FUNDER_ADDRESS: The proxy wallet address shown in your Polymarket settings
        ...
        # Try to derive existing creds first, otherwise create new ones
        try:
...
def calculate_arbitrage_guaranteed(p_yes, p_no, total_capital):
    ...
    if combined_prob <= 0 or combined_prob >= 1.0: return None # No arb if combined prob > 1
    
    stake_yes = (p_no / combined_prob) * total_capital
    stake_no = (p_yes / combined_prob) * total_capital
    
    if stake_yes < 5.0 or stake_no < 5.0: return None # Polymarket min order is often $5
        ...
        try:
            ...
            for e in resp.json():
                ...
                if m_data and 'YES' in m_data and 'NO' in m_data:
                    ...
                    if arb and arb['roi'] > 0.1: # Only cache if actually profitable
                        ARBI_CACHE.append({
...
async def handle_query(update, context):
    ...
    if "SET_" in q.data:
        ...
        results = []
        # Polymarket Market Buy: 'amount' is total USDC to spend
        orders = [
            (target['yes_id'], calc['stake_yes']),
            (target['no_id'], calc['stake_no'])
        ]
        
        for (t_id, amt) in orders:
            try:
