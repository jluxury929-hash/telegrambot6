...
async def run_atomic_execution(context, chat_id, side):
    ...
    # Task 2: Background Transaction Signing (signed while Sim is counting down)
    prep_task = asyncio.create_task(prepare_signed_tx(w3.to_wei(0.001, 'ether')))
    ...
    # This fires immediately after the simulation ends
    await asyncio.sleep(0.001)
    
    # BROADCAST: Use raw_transaction (Snake_case fix)
...
async def handle_interaction(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("SET_"):
        ...
        # Trigger the 1ms Simultaneous Execution Engine
