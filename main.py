...
def init_clob():
    try:
        # FIX: Default to 0 (EOA) for standard wallets to prevent signature errors
...
def calculate_arbitrage_guaranteed(p_yes, p_no, total_capital):
    ...
    if combined_prob <= 0 or combined_prob >= 1.0: return None # Must be < 1.0 for guaranteed profit
...
async def start(update, context):
    ...
    welcome_text = (f"{LOGO}\n<b>HYDRA ARBITRAGE SYSTEM ONLINE</b>\n<i>Aave V3 Credit-Line Integration Active.</i>")
...
async def main_handler(update, context):
    ...
    if 'START ARBI-SCAN' in cmd:
        ...
        if await scour_arbitrage():
            ...
            await m.edit_text("⚠️ <b>NO < 3-DAY ARBS DETECTED.</b>")
    elif 'VAULT' in cmd:
        ...
        msg = (f"<b>VAULT AUDIT</b>\n━━━━━━━━━━━━━━\n"
               ...
               f"<b>Total Buying Power:</b> ${bal + avail_borrow:.2f}")
        ...
        await update.message.reply_text(f"<b>CALIBRATE STRIKE CAPITAL</b>\nSelect amount to utilize.\n<i>Max Available: ${total_pwr:.2f}</i>", reply_markup=InlineKeyboardMarkup(kb), parse_mode='HTML')
...
async def handle_query(update, context):
    ...
    if "SET_" in q.data:
        ...
        # Execute orders for both sides of the arbitrage
        for (t_id, amt) in [(target['yes_id'], calc['stake_yes']), (target['no_id'], calc['stake_no'])]:
            try:
                # MarketOrderArgs calculates the hash based on token and amount. 
                # Price is ignored in market orders but used for signature bounds internally.
