...
# RAILWAY PROXY CONFIG: Use PROXY_URL variable in Railway (e.g. http://user:pass@host:port)
    ...
    for url in endpoints:
        ...
        try:
            ...
            if _w3.is_connected():
                ...
                print(f"✅ Connected to RPC: {url[:25]}...")
                return _w3
        except: continue
    return None
...
w3 = get_hydra_w3()
if not w3:
    print("FATAL: RPC Failure. Retrying in 10s..."); time.sleep(10)
...
def calculate_arbitrage_guaranteed(p_yes, p_no, total_capital):
    # FIX: Price(0) Error - Never divide by zero or very low liquidity
    if p_yes <= 0.005 or p_no <= 0.005: return None
    combined_prob = p_yes + p_no
    if combined_prob >= 0.999: return None # No profit found
...
async def start(update, context):
    ...
    await update.message.reply_text(f"{LOGO}\n<b>HYDRA ULTRA ONLINE</b>", reply_markup=ReplyKeyboardMarkup(btns, resize_keyboard=True), parse_mode='HTML')
...
async def handle_query(update, context):
    ...
    if "SET_" in q.data:
        ...
        await q.edit_message_text(f"<b>{target['title']}</b>\nROI: {calc['roi']}%", reply_markup=InlineKeyboardMarkup(kb), parse_mode='HTML')
    elif "EXE_" in q.data:
        target = ARBI_CACHE[int(q.data.split("_")[1])]
        calc = calculate_arbitrage_guaranteed(target['p_y'], target['p_n'], stake)
        for (t_id, amt) in [(target['yes_id'], calc['stake_yes']), (target['no_id'], calc['stake_no'])]:
            try:
                order = clob_client.create_order(MarketOrderArgs(token_id=str(t_id), amount=float(amt), side="BUY"))
                clob_client.post_order(order, OrderType.FOK)
            except Exception as e: print(f"Trade Error: {e}")
        await context.bot.send_message(q.message.chat_id, "✅ <b>SENT TO POLYMARKET</b>")
...
async def ghost_pilot(app):
    while True:
        try:
            if await scour_arbitrage():
                # Auto-trading logic could be added here
                pass
            await asyncio.sleep(60)
        except: await asyncio.sleep(20)

# --- 7. MODERN ASYNC RUNNER ---
async def main():
    print("🚀 [HYDRA] Booting Ultra Engine...")
    ...
    async with app:
        ...
        await app.start()
        # drop_pending_updates=True is the cure for the 'Conflict' error
        await app.updater.start_polling(drop_pending_updates=True)
        print("✅ Hydra Online and Monitoring.")
