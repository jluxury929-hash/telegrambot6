...
# FIX: Updated ABI for allowance/approve
...
# FIX: Start-up Auto-Approve logic
def preflight_approve():
    if usdc_contract.functions.allowance(vault.address, CTF_EXCHANGE).call() < 10**12:
        print("🛠️ APPROVING USDC...")
...
async def handle_callback(update, context):
    ...
    if "SET_" in query.data:
        ...
        try:
            ...
            # FIX: Included errorMsg for better debugging
            await context.bot.send_message(query.message.chat_id, WIN_LOGO if resp.get("success") else f"{LOSE_LOGO}\n<code>{resp.get('errorMsg', 'Market Error')}</code>", parse_mode='HTML')
        except Exception as e:
            await context.bot.send_message(query.message.chat_id, f"☢️ <b>ERROR:</b> <code>{str(e)[:50]}</code>", parse_mode='HTML')

if __name__ == "__main__":
    preflight_approve() # FIX: Added trigger
