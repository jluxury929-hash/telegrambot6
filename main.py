...
def calculate_arbitrage_guaranteed(p_yes, p_no, total_capital):
    ...
    if combined_prob >= 1.0 or combined_prob <= 0: return None # Strictly Profitable
...
async def scour_arbitrage():
    ...
    # EXPANDED SEARCH: Scrape multiple endpoints to get "tons of options"
    endpoints = [
        ...
        "https://gamma-api.polymarket.com/events?active=true&closed=false&limit=50&tag_id=10015" # Sports Example
    ]
...
async def start(update, context):
    btns = [['🚀 SCAN PROFITABLE', '📊 CALIBRATE'], ['💳 VAULT', '🔧 FIX APPROVAL']]
    await update.message.reply_text(f"{LOGO}\n<b>MAX-PROFIT ENGINE ACTIVE</b>\nFiltering: Same-Day, ROI > 0%.", reply_markup=ReplyKeyboardMarkup(btns, resize_keyboard=True), parse_mode='HTML')

async def main_handler(update, context):
    cmd = update.message.text
    if 'SCAN PROFITABLE' in cmd:
        m = await update.message.reply_text("📡 <b>SCANNING 150+ MARKETS...</b>", parse_mode='HTML')
        if await scour_arbitrage():
            kb = [[InlineKeyboardButton(f"🟢 {a['roi']}% - {a['title']}", callback_data=f"ARB_{i}")] for i, a in enumerate(ARBI_CACHE[:15])]
            await m.edit_text(f"<b>FOUND {len(ARBI_CACHE)} PROFITABLE BETS:</b>", reply_markup=InlineKeyboardMarkup(kb), parse_mode='HTML')
        else:
            await m.edit_text("🛰 <b>NO PROFITABLE SAME-DAY BETS FOUND.</b>\nTry again in 5 minutes.")
...
async def handle_query(update, context):
    ...
    if "SET_" in q.data:
        ...
        msg = f"<b>PROFIT PLAN:</b> {target['title']}\n\n✅ YES: ${calc['stake_yes']}\n❌ NO: ${calc['stake_no']}\n💰 ROI: {calc['roi']}%"
        kb = [[InlineKeyboardButton("⚡ EXECUTE ARBITRAGE", callback_data=f"EXE_{q.data.split('_')[1]}")]]
        await q.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(kb), parse_mode='HTML')
    elif "EXE_" in q.data:
        # (Execution code remains same, wrapped in results check)
        target = ARBI_CACHE[int(q.data.split("_")[1])]
        calc = calculate_arbitrage_guaranteed(target['p_y'], target['p_n'], stake)
        results = []
        ...
        await context.bot.send_message(q.message.chat_id, "✅ <b>PROFIT SECURED</b>" if all(results) else "⚠️ <b>EXECUTION ERROR</b>", parse_mode='HTML')
