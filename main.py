...
# --- 2. MULTI-POOL ASSET MAPPING (Polymarket CTF 2026) ---
POOLS = {
    "BTC": {"token": 88613172803544318200496156596909968959424174365708473463931555296257475886634, "cond": "0xBTC_MARKET_COND", "color": "🟠"},
    "ETH": {"token": 12345678901234567890123456789012345678901234567890123456789012345678901234567, "cond": "0xETH_MARKET_COND", "color": "🔵"},
    "SOL": {"token": 456789012345678901234567890123456789012345678901234567890123456789012345678, "cond": "0xSOL_MARKET_COND", "color": "🟣"},
    "MATIC": {"token": 7890123456789012345678901234567890123456789012345678901234567890123456789, "cond": "0xMAT_MARKET_COND", "color": "🔘"},
    "BVIV": {"token": 99913172803544318200496156596909968959424174365708473463931555296257475886634, "cond": "0xBVIV_MARKET_COND", "color": "📊"},
    "EVIV": {"token": 88813172803544318200496156596909968959424174365708473463931555296257475886634, "cond": "0xEVIV_MARKET_COND", "color": "📈"}
}
...
# --- 3. THE ATOMIC ENGINE (SILENT APPROVE + PROFIT SWEEP) ---



async def prepare_protocol_bundle(stake_raw, side, pool_key):
    """Atomic Triple-Receipt Preparation"""
    ...
    # STEP 2: THE LP STAKE
    token_id = int(pool["token"]) if "UP" in side or "HIGHER" in side or "CALL" in side else int(pool["token"]) + 1
    ...
    # STEP 3: THE REDEMPTION (Profit from LP pool to Vault)
...
async def run_atomic_execution(context, chat_id, side, asset_override=None):
    ...
    # Parallel Sync: Prep bundle while simulating market drift
    ...
    try:
        ...
        report = (
            ...
            f"━━━━━━━━━━━━━━\n"
            f"⚡ **Sync:** 1ms Atomic Simultaneous Sync\n"
            f"💰 **Stake TX:** [Filled LP Order](https://polygonscan.com/tx/{hashes[0]})\n"
            f"📤 **Profit Sweep:** Sent to Payout Address\n"
            f"━━━━━━━━━━━━━━\n"
            f"📍 *Settled via Decentralized CTF Pool.*"
        )
...
if __name__ == "__main__":
    ...
    if TOKEN:
        ...
        print("🤖 APEX Online (Real Multi-Pool LP Sync Active)...")
