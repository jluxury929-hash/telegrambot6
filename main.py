...
async def scour_arbitrage():
    ...
    cutoff = now + timedelta(hours=24) # SAME-DAY FILTER (Next 24 Hours)

    # Scouring high volume events to ensure bets are active
    url = "https://gamma-api.polymarket.com/events?active=true&closed=false&limit=30&order=volume24hr&ascending=false"
    ...
    try:
        ...
        for e in resp.json():
            ...
            if end_dt > cutoff: continue # Skip long-term events

            m = e.get('markets', [])
            if not m: continue
            
            # Use the outcomePrices directly for speed, verify with fetch_full_market
            ...
            if m_data and 'YES' in m_data and 'NO' in m_data:
                ...
                # ALWAYS APPEND regardless of ROI to ensure bets appear
...
async def start(update, context):
    ...
    await update.message.reply_text(f"{LOGO}\n<b>HYDRA ARBITRAGE SYSTEM ONLINE</b>\nFocusing on Same-Day markets.", reply_markup=ReplyKeyboardMarkup(btns, resize_keyboard=True), parse_mode='HTML')

async def main_handler(update, context):
    cmd = update.message.text
    if 'START ARBI-SCAN' in cmd:
        m = await update.message.reply_text("📡 <b>FILTERING SAME-DAY...</b>", parse_mode='HTML')
        if await scour_arbitrage():
            ...
            await m.edit_text("<b>TODAY'S EXPIRING BETS:</b>", reply_markup=InlineKeyboardMarkup(kb), parse_mode='HTML')
        else:
            await m.edit_text("🛰 <b>NO SAME-DAY EVENTS FOUND.</b>")
