...
async def main_handler(update, context):
    ...
    if 'START ARBI-SCAN' in cmd:
        ...
        kb = [[
            InlineKeyboardButton(f"${x}",callback_data=f"SET_{x}")
            for x in [5,10,50,100,250,500]
        ]]
...
async def handle_query(update, context):
    ...
    if "SET_" in q.data:
        ...
        await q.edit_message_text(
            f"✅ <b>CAPITAL LOADED: ${context.user_data['stake']}</b>"
        )
    ...
    if "ARB_" in q.data:
        ...
        calc = calculate_arbitrage_guaranteed(
            target['p_y'],
            target['p_n'],
            stake
        )
        ...
        await q.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("🔥 EXECUTE",callback_data=f"EXE_{idx}")]]
            ),
            parse_mode='HTML'
        )
        ...
        try:
            ...
            if not raw_data:
                raise Exception("Market data unreachable")
            ...
            ob = OrderBuilder(
                client.get_address(),
                137,
                int(os.getenv("SIGNATURE_TYPE",1))
            )

            ob.funder = os.getenv("FUNDER_ADDRESS",vault.address)

            ob.contract_address = (
                NEG_RISK_EXCHANGE if target['neg_risk']
                else CTF_EXCHANGE
            )
        ...
        status = (
            "✅ <b>ARBITRAGE SECURED</b>"
            if not err_msg
            else f"⚠️ <b>EXE ERROR</b>\n<code>{err_msg}</code>"
        )
